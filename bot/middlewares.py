    ...
    async def _process_message_throttling(self, name: str, bot: Bot, message: Message):
        ...
        if message.chat.type == 'private':
            if self.storage[name][key] == number_of_messages + 1:
                # Here you can notify the user that he has exceeded the message processing limit.
                pass

        elif message.chat.type in {'group', 'supergroup'}:
            if self.storage[name][key] == number_of_messages + 1:
                # Here you can notify the user that he has exceeded the limit or block him in the chat.
                pass
